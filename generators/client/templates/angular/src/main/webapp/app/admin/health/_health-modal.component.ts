...
export class <%=jhiPrefixCapitalized%>HealthModalComponent implements OnInit {
    ...
    getDatabaseInfo(){
        var formattedHelathName = JSON.stringify(this.currentHealth.name).replace('.','')

        this.healthService.getDatabaseInfo(formattedHelathName).subscribe(
            (res: HttpResponse<any>) => {
                this.databaseDetails(res.body)
            },
            (res: HttpErrorResponse) => console.log('Error'),
        );
    }

    databaseDetails(data: any) {
        this.dbserver = data.databaseServer;
